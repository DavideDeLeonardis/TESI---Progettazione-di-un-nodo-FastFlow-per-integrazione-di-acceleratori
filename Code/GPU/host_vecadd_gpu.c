...
int main(int argc, char** argv) {
	...
	// Load kernel from file kernel_vecadd_gpu.cl
	FILE* kernelFile;
	char* kernelSource;
	size_t kernelSize;

	kernelFile = fopen("kernel_vecadd_gpu.cl", "r");

	if (!kernelFile) {
		fprintf(stderr, "No file named kernel_vecadd_gpu.cl was found\n");
  ...
  // Memory buffers for each array
	cl_mem aMemObj = clCreateBuffer(context, CL_MEM_READ_ONLY,
											  SIZE * sizeof(float), NULL, &ret);
	cl_mem bMemObj = clCreateBuffer(context, CL_MEM_READ_ONLY,
											  SIZE * sizeof(float), NULL, &ret);
	cl_mem cMemObj = clCreateBuffer(context, CL_MEM_WRITE_ONLY,
											  SIZE * sizeof(float), NULL, &ret);

	// Copy lists to memory buffers
	ret = clEnqueueWriteBuffer(commandQueue, aMemObj, CL_TRUE, 0,
										SIZE * sizeof(float), A, 0, NULL, NULL);
	ret = clEnqueueWriteBuffer(commandQueue, bMemObj, CL_TRUE, 0,
  ...
  // Build program
	...
	size_t localItemSize =
		64; // globalItemSize has to be a multiple of localItemSize. 1024/64 = 16
	...
	for (i = 0; i < SIZE; ++i) {
		if (C[i] != (A[i] + B[i])) {
			printf("Something didn't work correctly! Failed test. (%f = %f) \n", C[i],
					 A[i] + B[i]);
			break;
			}
		}
