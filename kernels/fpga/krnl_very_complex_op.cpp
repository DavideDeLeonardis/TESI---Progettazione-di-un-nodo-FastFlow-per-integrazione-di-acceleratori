...
Description:
    ...
    This kernel implements a computationally-intensive and data-dependent
    operation using the load/compute/store coding style for Vitis HLS. It
    follows a dataflow architecture where data is streamed between three
    distinct stages:
    1. load_input: Reads data from global memory into streams.
    2. compute_very_complex: Performs the core iterative calculation.
    ...
    to operate in parallel for maximum throughput on the FPGA.
...
/**
 * @brief Reads from input streams, performs the complex iterative calculation,
 ...
 * @param size The number of elements to process.
 */
static void compute_very_complex(hls::stream<int32_t> &in1_stream,
...
#pragma HLS LOOP_TRIPCOUNT min = c_size max = c_size
#pragma HLS PIPELINE II = 1
      int32_t val_a_32 = in1_stream.read();
      int32_t val_b_32 = in2_stream.read();

      int64_t val_a = val_a_32;
      int64_t val_b = val_b_32;

      int64_t result = (3 * val_a * val_a) - (2 * val_b * val_b);

   iter_loop:
      for (int j = 0; j < 5; ++j) {
#pragma HLS UNROLL
         int64_t abs_val_b = (val_b < 0) ? -val_b : val_b;
         int64_t divisor = abs_val_b + 1;
 ...
 */
void krnl_very_complex_op(int32_t *in1, int32_t *in2, int32_t *out, int size) {
...
#pragma HLS dataflow
   ...
   compute_very_complex(in1_stream, in2_stream, out_stream, size);
