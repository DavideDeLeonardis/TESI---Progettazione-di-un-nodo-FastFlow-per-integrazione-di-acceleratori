...
bool FpgaAccelerator::initialize() {
   ...
   // Nota: Usiamo CL_DEVICE_TYPE_ACCELERATOR invece di CL_DEVICE_TYPE_GPU
   ...
   // --- DIFFERENZA CHIAVE: Caricamento del file binario .xclbin ---
   std::ifstream binaryFile("krnl_vadd.xclbin", std::ios::binary);
   if (!binaryFile.is_open()) {
      std::cerr << "Error: Could not open kernel file krnl_vadd.xclbin\n";
      return false;
   }
   ...
   if (!program_ || ret != CL_SUCCESS) {
      std::cerr << "Error: Failed to create program from binary\n";
      return false;
   }

   // NON serve clBuildProgram() perché il programma è già compilato!
   // -------------------------------------------------------------------

   // Usiamo il nome corretto del kernel
...
void FpgaAccelerator::execute(void *generic_task, long long &computed_us) {
   ...
   // --- PRIMA MODIFICA CHIAVE: Correzione del tipo per 'n' ---
   // Il kernel si aspetta un 'int' (32 bit), ma task->n è 'size_t' (64 bit).
   // Creiamo una variabile del tipo corretto da passare al kernel.
   int n_as_int = static_cast<int>(task->n);

   // Imposta gli argomenti del kernel (in1, in2, out, size)
   ...
   clSetKernelArg(kernel_, 3, sizeof(int),
                  &n_as_int); // Passiamo la variabile corretta

   // --- SECONDA MODIFICA CHIAVE: Esplicitare il local_work_size ---
   size_t global_work_size = 1;
   size_t local_work_size = 1; // Rendiamo esplicito anche questo
